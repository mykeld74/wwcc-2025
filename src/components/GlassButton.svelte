...
<style>
	...
	.button-wrap {
		...
		pointer-events: none;
		transition: all var(--animHoverTime) var(--animHoverEase);
	}
	...
	.button-shadow {
		...
		position: absolute;
		width: calc(280px + var(--shadow-cuttoff-fix));
	...
	button p::after {
		...
		transition:
			background-position calc(var(--animHoverTime) * 1.25) var(--animHoverEase),
			--angle-2 calc(var(--animHoverTime) * 1.25) var(--animHoverEase);
	}
	...
	button::after {
		...
		mask-composite: exclude;
		transition:
			all var(--animHoverTime) var(--animHoverEase),
			--angle-1 500ms ease;
	...
	.button-wrap:has(button:hover) .button-shadow {
		...
		transition: filter var(--animHoverTime) var(--animHoverEase);
	}
